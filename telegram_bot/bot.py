...
from telegram_bot import teclados
from telegram_bot.notificaciones import enviar_error, enviar_resultado
...
def construir_app(settings: Settings) -> Application:
    ...
    async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        await update.effective_message.reply_text(
            ...
            "/menu — activar o desactivar casas y deportes\n"
    ...
    async def cmd_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        await update.effective_message.reply_text(
            "Elige una casa de apuestas:", reply_markup=teclados.teclado_casas()
        )
    ...
    async def cb_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query
        await query.answer()
        _, casa_id, deporte = query.data.split(":")
    ...
    @solo_admin
    async def cb_volver_casas(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query
        await query.answer()
        await query.edit_message_text("Elige una casa de apuestas:", reply_markup=teclados.teclado_casas())
    ...
    app.add_handler(CallbackQueryHandler(cb_volver_casas, pattern=r"^volver_casas$"))
    return app
